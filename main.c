...
//    init_inecho();
    ...
    while (1) //Do this when you want an infinite loop of code
    {
        ...
        while (mode == 0)
        {
            col1 = GPIO_getInputPinValue(COL_1_PORT, COL_1_PIN);
            col2 = GPIO_getInputPinValue(COL_2_PORT, COL_2_PIN);
            col3 = GPIO_getInputPinValue(COL_3_PORT, COL_3_PIN);
            if (col1 == GPIO_INPUT_PIN_HIGH && keypadState == 0)
            {
                keypadState = 1;
                keyVal = getKeypadValue(outputRow, 0);
                calibrate_value = update_calibrate_value(keyVal, calibrate_value);
                GPIO_setOutputHighOnPin(LED2_PORT, LED2_PIN);
                __delay_cycles(300000);
            }
            else if (col2 == GPIO_INPUT_PIN_HIGH && keypadState == 0)
            {
                keypadState = 1;
                keyVal = getKeypadValue(outputRow, 1);
                calibrate_value = update_calibrate_value(keyVal, calibrate_value);
                GPIO_setOutputHighOnPin(LED2_PORT, LED2_PIN);
                __delay_cycles(300000);
            }
            else if (col3 == GPIO_INPUT_PIN_HIGH && keypadState == 0)
            {
                keypadState = 1;
                keyVal = getKeypadValue(outputRow, 2);
                calibrate_value = update_calibrate_value(keyVal, calibrate_value);
                GPIO_setOutputHighOnPin(LED2_PORT, LED2_PIN);
                __delay_cycles(300000);
            }
            ...
            outputRow = (outputRow + 1) % 4;
            GPIO_setOutputLowOnPin(output_row_port, output_row_pin);
            output_row_port = getNextKeypadInputPort(outputRow);
            output_row_pin = getNextKeypadInputPin(outputRow);
            GPIO_setOutputHighOnPin(output_row_port, output_row_pin);
        }
